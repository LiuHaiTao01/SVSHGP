...
class HeteroGaussian(Likelihood):
    ...
    def predict_mean_and_var(self, Fmu, Fvar, Fmu_g, Fvar_g):
        return tf.identity(Fmu), Fvar + tf.exp(Fmu_g + 0.5 * Fvar_g)

    @params_as_tensors
    def variational_expectations(self, Fmu, Fvar, Fmu_g, Fvar_g, Y):
        Rg = tf.exp(Fmu_g - 0.5 * Fvar_g)
        return -0.5 * np.log(2 * np.pi) - 0.5 * tf.log(Rg) \
               - 0.5 * (tf.square(Y - Fmu) + Fvar) / Rg - 0.25 * Fvar_g 
...
class RobustMax(Parameterized):
    ...
    def prob_is_largest(self, Y, mu, var, gh_x, gh_w):
        ...
        # (lht): if y is the observed class, then pick up mu^y and var^y
